...
if(isset($_GET['search']))
	...
	$get_allpro="select * from products where product_keyword like '%$query%'";
	$run_pro=mysqli_query($con,$get_allpro);
	$count_brand=mysqli_num_rows($run_pro);
	  if ($count_brand==0)
	  {echo "<h1><center>You Search For $query.</br>Soory! No Products Found of Keyword.</center></h1>";
	  }
	  else
	  {
	while($row_pro=mysqli_fetch_array($run_pro))
		{
		    ...
		    <h1><center>You Search For $query</center></h1>
			<h3>$pro_title</h3>
